...
```{r}
class(data)
data_notas_altas <- data[data$Previous_Scores >= 80,]
data_notas_altas
```
...
```{r}
data_notas_altas <- clean_and_filter_data(data_notas_altas)
data_notas_altas
```
...
```{r}
par(mfrow=c(2,2))
for(name1 in colnames(data_notas_altas)) {
  for(name2 in colnames(data_notas_altas)) {
    if(name1 != "Previous_Scores" && name2 != "Previous_Scores" && name1 != name2) {
      interaction.plot(data_notas_altas$name1,data_notas_altas$name2,data_notas_altas$Previous_Scores)
