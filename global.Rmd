...
for (i in 1:length(col_names)) {
  for (j in (i+1):length(col_names)) {  # Evita repetir pares en orden inverso
    name1 <- col_names[i]
    name2 <- col_names[j]
    
    if (name1 != "Previous_Scores" && name2 != "Previous_Scores") {
      interaction.plot(
        data_notas_altas[[name1]],  # Convertir a factor
        data_notas_altas[[name2]],  
...
par(mfrow=c(1,1))  # Restablece la disposición de la ventana gráfica
